...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //2) Check if user exists and passwordis correct

  //using +password because password in model was defined the select: false
...
exports.isLoggedIn = async (req, res, next) => {
  try {
    if (req.cookies.jwt) {
      ...
      //There is a logged in user || res.locals.user that means inside of template, there will be a variable called user and each pug template has ability to access to the req.locals
      res.locals.user = currentUser;
      return next();
    }
  } catch (err) {
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  //the req.user.id coming from authController.protect
